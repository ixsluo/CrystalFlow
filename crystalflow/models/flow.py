...
class FlowModule(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.cost = cost
        self.mode = self.types_probpath.mode
        assert self.mode in ["CSP", "DNG"], f"Unknown atom type mode: {self.mode}"

        self.vector_field = vector_field(
            types_input_dim=self.types_probpath.output_dim,
            types_output_dim=self.types_probpath.decode_dim,
            lattice_input_dim=self.lattice_probpath.output_dim,
            lattice_output_dim=self.lattice_probpath.decode_dim,
        )
    ...
    def forward(self, batch):
        ...
        if self.mode == "CSP":
            ...
            return {"loss": loss, "loss_lattice": loss_lattice, "loss_frac_coords": loss_frac_coords}
        elif self.mode == "DNG":
